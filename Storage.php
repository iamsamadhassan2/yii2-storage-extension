...
/**
 * The Storage class manages the storage medium for files that are uploaded.
 *
 * Handles 4 types of storage:
 * - Amazon S3
 * - Azure Storage Blob
 * - Google Storage Cloud
 * - Storage local.
 * @author Néstor Acevedo (improved by Abdul samad hassan)
 */
class Storage extends \yii\base\Component
{

    /**
     * Storage constant: Amazon S3
     */
    const AWS_S3 = 's3';

    /**
     * Storage constant: Azure Storage Blob
     */
    const AZURE_BLOB_STORAGE = 'azure';

    /**
     * Storage constant: Google Storage Cloud
     */
    ...
    /**
     * @var mixed Instancia de almacenamiento (S3Client, BlobRestProxy, StorageClient).
     * @access private
     */
    ...
    private function uploadToAzure($file)
    {
        try {
            if ($file->validate()) {
                ...
                $this->clientService->createBlockBlob($this->bucket, $this->prefix . $file->uploadedFile->name, $blob_content, $blockBlobOptions);
                return true;
            } else {
                return false;
            }
        ...
        try {
            if ($file->validate()) {
                ...
                $this->clientService->bucket($this->bucket, true)->upload($content, [
                    'name' => $this->prefix . $file->uploadedFile->name, 'predefinedAcl' => 'publicRead']);
                return true;
            } else {
                return false;
            }
